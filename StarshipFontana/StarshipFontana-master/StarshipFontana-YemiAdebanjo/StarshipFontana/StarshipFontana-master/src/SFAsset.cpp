...
SFAsset::SFAsset(SFASSETTYPE type, std::shared_ptr<SFWindow> window): type(type), sf_window(window) {
  ...
  case SFASSET_COIN:
    ...
    break;
  case SFASSET_BOTTOMWALL:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/bottomwall.png");
    break;
  case SFASSET_LEFTWALL:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/leftwall.png");
    break;
  case SFASSET_TOPWALL:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/topwall.png");
    break;
  case SFASSET_RIGHTWALL:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/rightwall.png");
    break;
 ...
 case SFASSET_GAMEOVER:
    ...
    break;
   case SFASSET_MOTHERSHIP:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/mothership.png");

//uses the game's "assets" folder to determine what the game objects look like in-game
...
//used for mothership movement


// this is utilised if there is a collision between the player and the game's "west" wall
void SFAsset::WestWallCollision() {
...
//this is primarily used by the mothership


void SFAsset::EastWallCollision() {
  int w, h;
  SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
...
// this is utilised if there is a collision between the player and the game's "east" wall
...
void SFAsset::NorthWallCollision(){
 int w, h; 
SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
  Vector2 c = *(bbox->centre) + Vector2(0.0f, 10.0f);
if(!((c.getY()+bbox->extent_y->getY()) > h)) {                    // this is utilised if there is a collision between the player and the game's "north" wall
  bbox->centre.reset();
  bbox->centre = make_shared<Vector2>(c);
}
}
...
void SFAsset::PlayerBulletNorth() {
  Vector2 c = *(bbox->centre) + Vector2(0.0f, 10.0f);
  bbox->centre.reset();
  bbox->centre = make_shared<Vector2>(c);
}
//used to handle player projectile speed


void SFAsset::AlienBulletSouth() {
  Vector2 c = *(bbox->centre) + Vector2(0.0f, -10.0f);
  bbox->centre.reset();
  bbox->centre = make_shared<Vector2>(c);
}
//used to handle alien projectile speed
...
void SFAsset::HandleCollision() {
  if(SFASSET_PROJECTILE == type || SFASSET_PLAYER == type || SFASSET_COIN == type || SFASSET_ALIEN == type || SFASSET_TOPWALL == type || SFASSET_BOTTOMWALL == type || SFASSET_LEFTWALL == type || SFASSET_RIGHTWALL == type || SFASSET_ASTEROID == type || SFASSET_MOTHERSHIP == type || SFASSET_GAMEOVER == type) {
    SetNotAlive();
  }
